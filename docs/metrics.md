...
| **Questions per resolution**| Breaking questions asked per resolved session (rolling avg)   | trending ↓|
...
**Questions per resolution is the primary learning KPI.** A graph that is
genuinely learning should show this number trending downward over time as
high-confidence paths eliminate the need for disambiguation. A flat or rising
trend signals that reinforcement is not compounding — usually caused by
conflicting session outcomes or under-weighted paths. The target is not a
fixed number but a negative slope over a rolling window of sessions.
